...
function kitecom_template_options() {
	...
	include_once(kitecom_file('kitecom/layouts/template.php'));
}
...
function kitecom_template_options_js() {
	// variable used for the page detection
	global $pagenow;
	// template object
	global $tpl;
	// check the page
	if($pagenow == 'admin.php' && isset($_GET['page']) && ($_GET['page'] == 'template_options' || $_GET['page'] == 'kitecom-menu')) {
		wp_enqueue_script('media-upload');
		wp_enqueue_script('thickbox');
		wp_register_script('gk-tips-js', kitecom_file_uri('js/back-end/libraries/miniTip/miniTip.min.js'), array('jquery'));
		wp_register_script('gk-upload', kitecom_file_uri('js/back-end/template.options.js'), array('jquery','media-upload','thickbox', 'gk-tips-js'));
						...
						foreach($group->fields as $field) {
							if(!in_array($field->type, $standard_fields)) {
								...
								if((is_array($file_config) && count($file_config) > 0) || is_object($file_config)) {
									// load the JS file
									if($file_config->js != '') {
										wp_register_script('gk_'.strtolower($file_config->name).'.js', kitecom_file_uri('kitecom/form_elements/').($field->type).'/'.($file_config->js));
...
function kitecom_template_options_css() {
	// variable used for the page detection
	global $pagenow;
	// template object
	global $tpl;
	...
	if($pagenow == 'admin.php' && isset($_GET['page']) && ($_GET['page'] == 'template_options' || $_GET['page'] == 'kitecom-menu')) {
		...
		wp_register_style('gk-tips-css', kitecom_file_uri('js/back-end/libraries/miniTip/miniTip.css'));
		wp_register_style('gk-template-css', kitecom_file_uri('css/back-end/template.css'));
						...
						foreach($group->fields as $field) {
							if(!in_array($field->type, $standard_fields)) {
								...
								if((is_array($file_config) && count($file_config) > 0) || is_object($file_config)) {
									// load the CSS file
									if($file_config->css != '') {
										wp_register_style('gk_'.strtolower($file_config->name).'.css', kitecom_file_uri('kitecom/form_elements/').($field->type).'/'.($file_config->css));
...
function kitecom_template_save_js() {
	$ajax_nonce = wp_create_nonce('KitecomWPNonce');
...
function kitecom_template_save_callback() {
	// getting access to the template global object. 
	global $tpl;
	...
	if ( current_user_can( 'manage_options' ) ) {
 		...
 		if ( function_exists( 'check_ajax_referer' ) ) { 
 			check_ajax_referer( 'KitecomWPNonce', 'security' ); 
 		}
...
add_action( 'wp_ajax_template_save', 'kitecom_template_save_callback' );
