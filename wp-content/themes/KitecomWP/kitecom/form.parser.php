...
include_once(kitecom_file('kitecom/form_elements/standard.php'));
				...
				if(in_array($field->type, $standard_fields)) {
					...
					if((is_array($file_config) && count($file_config) > 0) || is_object($file_config)) {
						...
						if(!class_exists($file_config->class)) {
							...
							include_once(kitecom_file('kitecom/form_elements/').($field->type).'/'.($file_config->php));
						}
						// create the object
						if(class_exists($file_config->class)) {
