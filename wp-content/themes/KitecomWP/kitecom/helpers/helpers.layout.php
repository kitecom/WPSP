...
function gk_load($part_name, $assets = null, $args = null) {	
	...
	if($assets !== null) {
		foreach($assets as $key => $value) {
			if($key == 'css') {
				wp_enqueue_style('kitecom-gallery-template', $value, array('kitecom-stuff'));
			} elseif($key == 'js') {
				wp_enqueue_script('kitecom-gallery-template', $value, array('jquery'));
			}
		}
	}

	include(kitecom_file('layouts/' . $part_name . '.php'));
...
function gk_blog_name() {
	...
	if(get_option($tpl->name . '_seo_use_gk_seo_settings') == 'Y') {
		...
		return apply_filters('kitecom_blog_name', get_option($tpl->name . '_seo_blogname'));
	} else { // in other case
		// output standard blog name
		return apply_filters('kitecom_blog_name', get_bloginfo( 'name' ));
	}
}
...
function gk_blog_desc() {
	...
	if(get_option($tpl->name . '_seo_use_gk_seo_settings') == 'Y') {
		...
		return apply_filters('kitecom_blog_desc', get_option($tpl->name . '_seo_description'));
	} else { // in other case
		// output standard blog description
		return apply_filters('kitecom_blog_desc', get_bloginfo( 'description' ));
	}
}
 ...
 *
 **/
function kitecom_get_attachment_id( $attachment_url = '' ) {
	global $wpdb;
	$attachment_id = false;
...
function gk_blog_logo() {
	...
	if(get_option($tpl->name . "_branding_logo_type", 'css') == 'image') {
		...
		if(get_option($tpl->name . "_branding_logo_alt_image") != '') {
			...
			$image_id = kitecom_get_attachment_id($image_src);
		...
		// return the logo output
		echo apply_filters('kitecom_logo_html', $logo_text);
	}
}
	...
	if(get_option($tpl->name . '_seo_use_gk_seo_settings') == 'Y') {
		if(is_home() || is_front_page()) {
			if(get_option($tpl->name . '_seo_homepage_desc') == 'custom') {
				echo apply_filters('kitecom_meta_description', '<meta name="description" content="'.get_option($tpl->name . '_seo_homepage_desc_value').'" />');
			}
			
			if(get_option($tpl->name . '_seo_homepage_keywords') == 'custom') {
				echo apply_filters('kitecom_meta_keywords', '<meta name="keywords" content="'.get_option($tpl->name . '_seo_homepage_keywords_value').'" />');
			}
		}
		...
		if(is_singular()) {
			...
			if(get_post_meta($postID, 'kitecom-post-desc', true) != '') {
				if(get_option($tpl->name . '_seo_post_desc') == 'custom') {
					echo apply_filters('kitecom_meta_description', '<meta name="description" content="'.get_post_meta($postID, 'kitecom-post-desc',true).'" />');
				}
			}
			 			
			if(get_post_meta($postID, 'kitecom-post-keywords', true) != '') {
				if(get_option($tpl->name . '_seo_post_keywords') == 'custom') {
					echo apply_filters('kitecom_meta_keywords', '<meta name="keywords" content="'.get_post_meta($postID, 'kitecom-post-keywords',true).'" />');
				}
			}
		}
	}
...
function gk_opengraph_metatags() {
	...
	if(get_option($tpl->name . '_opengraph_use_opengraph') == 'Y') {
		if(is_single() || is_page()) {
			...
			$postID = $wp_query->post->ID;
			//
			$title = get_post_meta($postID, 'kitecom_opengraph_title', true);
			$type = get_post_meta($postID, 'kitecom_opengraph_type', true);
			$image = wp_get_attachment_url(get_post_meta($postID, 'kitecom_opengraph_image', true));
			...
			$desc = get_post_meta($postID, 'kitecom_opengraph_desc', true);
			$other = get_post_meta($postID, 'kitecom_opengraph_other', true);
			//
			echo apply_filters('kitecom_og_title', '<meta name="og:title" content="'.(($title == '') ? esc_html($wp_query->post->post_title) : $title).'" />' . "\n");
			//
			if($image != '') {
				echo apply_filters('kitecom_og_image', '<meta name="og:image" content="'.$image.'" />' . "\n");
			}
			//
			echo apply_filters('kitecom_og_type', '<meta name="og:type" content="'.(($type == '') ? 'article' : $type).'" />' . "\n");
			//
			echo apply_filters('kitecom_og_description', '<meta name="og:description" content="'.(($desc == '') ? substr(str_replace("\"", '', strip_tags($wp_query->post->post_content)), 0, 200) : $desc).'" />' . "\n");
			//
			echo apply_filters('kitecom_og_url', '<meta name="og:url" content="'.get_permalink($postID).'" />' . "\n");
			//
			if($other != '') {
				$other = preg_split('/\r\n|\r|\n/', $other);
				//
				foreach($other as $item) {
					...
					//	
					if(count($item) >= 2) {
						echo apply_filters('kitecom_og_custom', '<meta name="'.$item[0].'" content="'.$item[1].'" />' . "\n");
					}
				}
			}
		}
...
function gk_twitter_metatags() {
	...
	if(get_option($tpl->name . '_twitter_cards') == 'Y') {
		if(is_single() || is_page()) {
			...
			$postID = $wp_query->post->ID;
			//
			$title = get_post_meta($postID, 'kitecom_twitter_title', true);
			$image = wp_get_attachment_url(get_post_meta($postID, 'kitecom_twitter_image', true));
			...
			$desc = get_post_meta($postID, 'kitecom_twitter_desc', true);
			
			$site_default = get_option($tpl->name . '_twitter_site');
			$creator_default = get_option($tpl->name . '_twitter_creator');
			$site = get_post_meta($postID, 'kitecom_twitter_site', true);
			$creator = get_post_meta($postID, 'kitecom_twitter_creator', true);
			...
			echo apply_filters('kitecom_twitter_card', '<meta name="twitter:card" content="summary" />' . "\n");	
			//
			echo apply_filters('kitecom_twitter_url', '<meta name="twitter:url" content="'.get_permalink($postID).'" />' . "\n");
			//		
			echo apply_filters('kitecom_twitter_title', '<meta name="twitter:title" content="'.(($title == '') ? esc_html($wp_query->post->post_title) : $title).'" />' . "\n");
			//
			if($image != '') {
				echo apply_filters('kitecom_twitter_image', '<meta name="twitter:image" content="'.$image.'" />' . "\n");
			}
			echo apply_filters('kitecom_twitter_description', '<meta name="twitter:description" content="'.(($desc == '') ? substr(str_replace("\"", '', strip_tags($wp_query->post->post_content)), 0, 200) : $desc).'" />' . "\n");
			//
			echo apply_filters('kitecom_twitter_site', '<meta name="twitter:site" content="' . $site . '" />' . "\n");
			//
			echo apply_filters('kitecom_twitter_creator', '<meta name="twitter:creator" content="' . $creator . '" />' . "\n");
		}
	}
}
...
function gk_head_config() {
	...
	echo "           \$GK_TMPL_URL = '".kitecom_file_uri(false)."';\n";
...
function gk_breadcrumbs_output() {
	...
	if (!is_home()) {
		...
		$output .= '<a href="' . home_url() . '" class="gk-home">' . apply_filters('kitecom_breadcrumb_home', get_bloginfo('name')) . "</a>";
	...
	echo apply_filters('kitecom_breadcrumb', $output);
}
...
function gk_head_style_css() {
	...
	for($i = 0; $i < count($tpl->styles); $i++) {
		...
		wp_enqueue_style('kitecom-style', kitecom_file_uri('css/' . $url), array('kitecom-mobile'));
	}
}
...
function gk_head_shortcodes() {
	...
	if(get_option($tpl->name . "_shortcodes1_state", 'Y') == 'Y') {
		wp_enqueue_style('kitecom-shortcodes-typography', kitecom_file_uri('css/shortcodes.typography.css'), array('kitecom-extensions'));
		wp_enqueue_script('kitecom-shortcodes-typography', kitecom_file_uri('js/shortcodes.typography.js'), array('jquery', 'kitecom-scripts'), false, true);
	}
	// interactive
	if(get_option($tpl->name . "_shortcodes2_state", 'Y') == 'Y') {
		wp_enqueue_style('kitecom-shortcodes-elements', kitecom_file_uri('css/shortcodes.elements.css'), array('kitecom-extensions'));
		wp_enqueue_script('kitecom-shortcodes-elements', kitecom_file_uri('js/shortcodes.elements.js'), array('jquery', 'kitecom-scripts'), false, true);
	}
	// template
	if(get_option($tpl->name . "_shortcodes3_state", 'Y') == 'Y') {
		wp_enqueue_style('kitecom-shortcodes-template', kitecom_file_uri('css/shortcodes.template.css'), array('kitecom-extensions'));
		wp_enqueue_script('kitecom-shortcodes-template', kitecom_file_uri('js/shortcodes.template.js'), array('jquery', 'kitecom-scripts'), false, true);	
	}
}
	...
	for($i = 0; $i < count($tpl->fonts); $i++) {
		...
		if(trim($selectors) != '') {
			...
			if($type == 'normal') {
				...
				echo '<link href="' . kitecom_file_uri('fonts/' . $squirrel . '/stylesheet.css') . '" rel="stylesheet" type="text/css" />';
...
function gk_head_style_pages() {
	...
	if( is_page_template('contact.php') ){ 		
		wp_enqueue_script('kitecom-contact-validate', kitecom_file_uri('js/jquery.validate.min.js'), array('jquery', 'kitecom-scripts'), false, true);
		wp_enqueue_script('kitecom-contact-main', kitecom_file_uri('js/contact.js'), array('jquery', 'kitecom-scripts'), false, true);
	}
}
...
function gk_dynamic_sidebar($index) {
	...
	$counter = 0;
	// run hook
	do_action('kitecomwp_before_sidebar');
	...
	// run hook
	do_action('kitecomwp_after_sidebar');
}
...
function gk_thickbox_load() {
	...
	<?php
		wp_enqueue_style('kitecom-thickbox', home_url() . '/wp-includes/js/thickbox/thickbox.css', array('kitecom-extensions'));
		wp_enqueue_script('kitecom-thickbox', home_url() . '/wp-includes/js/thickbox/thickbox.js', array('jquery', 'kitecom-scripts'), false, true);
	endif;
}
