...
function kitecom_menu($menuname, $fullname, $params = null) {			
	global $tpl;
 ...
 *
 **/
function kitecom_comment_template( $comment, $args, $depth ) {
	global $tpl;
	
	$GLOBALS['comment'] = $comment;

	do_action('kitecomwp_before_comment');
	...
	do_action('kitecomwp_after_comment');
}
	...
	if(get_option($tpl->name . '_custom_fields_state', 'Y') == 'Y') {
		...
		if ($keys = get_post_custom_keys()) {
			...
			foreach ((array) $keys as $key) {
				...
				// skip the protected meta data and "kitecom-" or "kitecom_" values
				if(
					is_protected_meta($key_trimmed, 'post') || 
					stripos($key_trimmed, 'kitecom-') !== FALSE ||
					stripos($key_trimmed, 'kitecom_') !== FALSE ||
...
function gk_content_nav($id = '') {
	...
	if($wp_query->max_num_pages > 1) : ?>
		<?php do_action('kitecomwp_before_nav'); ?>
		...
		<?php do_action('kitecomwp_after_nav'); ?>
	<?php endif;
}
...
function gk_social_api($title, $postID) {
	...
	if($social_api_mode != 'none' && $is_excluded) {
		...
		if(get_option($tpl->name . '_pinterest_btn', 'Y') == 'Y') {
		     ...
		     $image = get_post_meta($postID, 'kitecom_opengraph_image', true);
		...
		$output = '<section id="gk-social-api">' . apply_filters('kitecom_social_api_fb', $fb_like_output) . apply_filters('kitecom_social_api_gplus', $gplus_output) . apply_filters('kitecom_social_api_twitter', $twitter_output) . apply_filters('kitecom_social_api_pinterest', $pinterest_output) . '</section>';
		
		return apply_filters('kitecom_social_api', $output);
	}
}
...
function gk_post_thumbnail_caption($raw = false) {
	...
	if ($thumbnail_img && isset($thumbnail_img[0])) {
		if($thumbnail_img[0]->post_excerpt != '') {
			if($raw) {
				return apply_filters('kitecom_thumbnail_caption', strip_tags($thumbnail_img[0]->post_excerpt));
			} else {
				return apply_filters('kitecom_thumbnail_caption', '<figcaption>'.$thumbnail_img[0]->post_excerpt.'</figcaption>');
			}
		}
	} else {
		return false;
