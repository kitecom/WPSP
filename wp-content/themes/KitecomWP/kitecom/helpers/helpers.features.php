...
/**
 *
 * KitecomWP admin panel & page features
 *
 * Functions used to create KitecomWP-specific functions 
 *
 **/
...
function add_kitecom_shortcode_button() {
    global $typenow;
	...
	if ( get_user_option('rich_editing') == 'true') {
		add_filter("mce_external_plugins", "add_kitecom_tinymce_plugin");
		add_filter('mce_buttons', 'register_kitecom_shortcode_button');
	}
}
// 
function register_kitecom_shortcode_button($buttons) {
   // add the shortcode button to the list
   array_push($buttons, "kitecom_shortcode_button");
   return $buttons;
}
// Load the plugin script
function add_kitecom_tinymce_plugin($plugin_array) {
	// add the shortcode button script to the list
   	$plugin_array['kitecom_shortcode_button'] = kitecom_file_uri('js/back-end/kitecom.shortcode.button.js');
   	return $plugin_array;
}
...
function add_kitecom_metaboxes() {
	global $tpl;
	...
	if(get_option($tpl->name . '_seo_use_gk_seo_settings') == 'Y' && get_option($tpl->name . '_seo_post_desc') == 'custom') {
		add_meta_box( 'kitecom-post-desc', __('Post keywords and description', GKTPLNAME), 'kitecom_post_seo_callback', 'post', 'normal', 'high' );
		add_meta_box( 'kitecom-post-desc', __('Page keywords and description', GKTPLNAME), 'kitecom_post_seo_callback', 'page', 'normal', 'high' );
	}
	// post title
	add_meta_box( 'kitecom-post-params', __('Post additional params', GKTPLNAME), 'kitecom_post_params_callback', 'post', 'side', 'default' );
	add_meta_box( 'kitecom-post-params', __('Page additional params', GKTPLNAME), 'kitecom_post_params_callback', 'page', 'side', 'default' );
}

function kitecom_post_seo_callback($post) { 
	$values = get_post_custom( $post->ID );  
	$value_desc = isset( $values['kitecom-post-desc'] ) ? esc_attr( $values['kitecom-post-desc'][0] ) : '';    
	$value_keywords = isset( $values['kitecom-post-keywords'] ) ? esc_attr( $values['kitecom-post-keywords'][0] ) : ''; 
	// nonce 
	wp_nonce_field( 'kitecom-post-seo-nonce', 'kitecom_meta_box_seo_nonce' ); 
    // output
    echo '<label for="kitecom-post-desc-value">'.__('Description:', GKTPLNAME).'</label>';
    echo '<textarea name="kitecom-post-desc-value" id="kitecom-post-desc-value" rows="5" style="width:100%;">'.$value_desc.'</textarea>'; 
    // output
    echo '<label for="kitecom-post-desc-value">'.__('Keywords:', GKTPLNAME).'</label>';
    echo '<textarea name="kitecom-post-keywords-value" id="kitecom-post-keywords-value" rows="5" style="width:100%;">'.$value_keywords.'</textarea>';    
} 

function kitecom_post_params_callback($post) { 
	$values = get_post_custom( $post->ID );  
	$value_title = isset( $values['kitecom-post-params-title'] ) ? esc_attr( $values['kitecom-post-params-title'][0] ) : 'Y';
	$value_image = isset( $values['kitecom-post-params-image'] ) ? esc_attr( $values['kitecom-post-params-image'][0] ) : 'Y'; 
	$value_templates = isset( $values['kitecom-post-params-templates'] ) ? $values['kitecom-post-params-templates'][0] : false; 
	...
	$value_aside = isset( $values['kitecom-post-params-aside'] ) ? $values['kitecom-post-params-aside'][0] : false;
	if($value_aside) {
		$value_aside = unserialize(unserialize($value_aside));
	}
	// nonce 
	wp_nonce_field( 'kitecom-post-params-nonce', 'kitecom_meta_box_params_nonce' ); 
    // output for the title option
    echo '<p>';
    echo '<label for="kitecom-post-params-title-value">'.__('Show title:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-title-value" id="kitecom-post-params-title-value">';
    ...
    echo '<p>';
    echo '<label for="kitecom-post-params-image-value">'.__('Show featured image:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-image-value" id="kitecom-post-params-image-value">';
    ...
    echo '<p>';
    echo '<label for="kitecom-post-params-aside-value">'.__('Show sidebar information:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-aside-value" id="kitecom-post-params-aside-value">';
    ...
    echo '</p>';
    echo '<p>';
    echo '<label for="kitecom-post-params-date-value">'.__('Show date:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-date-value" id="kitecom-post-params-date-value">';
    ...
    echo '</p>';
    echo '<p>';
    echo '<label for="kitecom-post-params-author-value">'.__('Show author:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-author-value" id="kitecom-post-params-author-value">';
    ...
    echo '</p>';
    echo '<p>';
    echo '<label for="kitecom-post-params-category-value">'.__('Show category:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-category-value" id="kitecom-post-params-category-value">';
    ...
    echo '</p>';
    echo '<p>';
    echo '<label for="kitecom-post-params-tags-value">'.__('Show tags:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-tags-value" id="kitecom-post-params-tags-value">';
    ...
    echo '</p>';
    echo '<p>';
    echo '<label for="kitecom-post-params-comments-value">'.__('Show comments:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-comments-value" id="kitecom-post-params-comments-value">';
    ...
    echo '<label for="kitecom-post-params-contact-name">'.__('Show name field:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-contact-name" id="kitecom-post-params-contact-name">';
    ...
    echo '</p>';
    echo '<p data-template="template.contact.php">';
    echo '<label for="kitecom-post-params-contact-email">'.__('Show e-mail field:', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-contact-email" id="kitecom-post-params-contact-email">';
    ...
    echo '</p>';  
    echo '<p data-template="template.contact.php">';
    echo '<label for="kitecom-post-params-contact-copy">'.__('Show "send copy":', GKTPLNAME).'</label>';
    echo '<select name="kitecom-post-params-contact-copy" id="kitecom-post-params-contact-copy">';
...
function kitecom_metaboxes_save( $post_id ) {  
    ...
    if( isset( $_POST['kitecom-post-desc-value'] ) ) {
        // check the nonce
        if( !isset( $_POST['kitecom_meta_box_seo_nonce'] ) || !wp_verify_nonce( $_POST['kitecom_meta_box_seo_nonce'], 'kitecom-post-seo-nonce' ) ) {
        	return;
        }
        // update post meta
        update_post_meta( $post_id, 'kitecom-post-desc', esc_attr( $_POST['kitecom-post-desc-value'] ) );  
    }
  	//
    if( isset( $_POST['kitecom-post-keywords-value'] ) ) {
    	// check the nonce
    	if( !isset( $_POST['kitecom_meta_box_seo_nonce'] ) || !wp_verify_nonce( $_POST['kitecom_meta_box_seo_nonce'], 'kitecom-post-seo-nonce' ) ) {
    		return;
    	}
    	// update post meta
        update_post_meta( $post_id, 'kitecom-post-keywords', esc_attr( $_POST['kitecom-post-keywords-value'] ) ); 
    }
    //
    if( isset( $_POST['kitecom-post-params-title-value'] ) ) {
    	// check the nonce
    	if( !isset( $_POST['kitecom_meta_box_params_nonce'] ) || !wp_verify_nonce( $_POST['kitecom_meta_box_params_nonce'], 'kitecom-post-params-nonce' ) ) {
    		return;
    	}
    	// update post meta
        update_post_meta( $post_id, 'kitecom-post-params-title', esc_attr( $_POST['kitecom-post-params-title-value'] ) ); 
    }
    //
    if( isset( $_POST['kitecom-post-params-aside-value'] ) ) {
    	// check the nonce
    	if( !isset( $_POST['kitecom_meta_box_params_nonce'] ) || !wp_verify_nonce( $_POST['kitecom_meta_box_params_nonce'], 'kitecom-post-params-nonce' ) ) {
    		return;
    	}
    	$aside_value = array(
    		'aside' => esc_attr( $_POST['kitecom-post-params-aside-value'] ),
    		'date' => esc_attr( $_POST['kitecom-post-params-date-value'] ),
    		'author' => esc_attr( $_POST['kitecom-post-params-author-value'] ),
    		'category' => esc_attr( $_POST['kitecom-post-params-category-value'] ),
    		'tags' => esc_attr( $_POST['kitecom-post-params-tags-value'] ),
    		'comments' => esc_attr( $_POST['kitecom-post-params-comments-value'] )
    	);
    	// update post meta
        update_post_meta( $post_id, 'kitecom-post-params-aside', serialize($aside_value) ); 
    }
    //
    if( isset( $_POST['kitecom-post-params-image-value'] ) ) {
    	// check the nonce
    	if( !isset( $_POST['kitecom_meta_box_params_nonce'] ) || !wp_verify_nonce( $_POST['kitecom_meta_box_params_nonce'], 'kitecom-post-params-nonce' ) ) {
    		return;
    	}
    	// update post meta
        update_post_meta( $post_id, 'kitecom-post-params-image', esc_attr( $_POST['kitecom-post-params-image-value'] ) ); 
    }
    //
    if( isset( $_POST['kitecom-post-params-contact-name'] ) ) {
    	// check the nonce
    	if( !isset( $_POST['kitecom_meta_box_params_nonce'] ) || !wp_verify_nonce( $_POST['kitecom_meta_box_params_nonce'], 'kitecom-post-params-nonce' ) ) {
    		return;
    	}
    	// update post meta
    	$contact_value = esc_attr( $_POST['kitecom-post-params-contact-name'] ) . ',' . esc_attr( $_POST['kitecom-post-params-contact-email'] ) . ',' . esc_attr( $_POST['kitecom-post-params-contact-copy'] );
    	$templates_value = array('contact' => $contact_value);
        update_post_meta( $post_id, 'kitecom-post-params-templates', serialize($templates_value) ); 
    }
}  
...
function kitecom_add_featured_video() {
	global $tpl;
	if(get_option($tpl->name . "_featured_video", 'Y') == 'Y') {
	    add_meta_box( 'kitecom_featured_video', __( 'Featured Video', GKTPLNAME ), 'kitecom_add_featured_video_metabox', 'post', 'side' );
	    add_meta_box( 'kitecom_featured_video', __( 'Featured Video', GKTPLNAME ), 'kitecom_add_featured_video_metabox', 'page', 'side' );
	}
}

function kitecom_add_featured_video_metabox() {
    global $post;

    $featured_video = get_post_meta($post->ID, '_kitecom-featured-video', true);
    
    echo '<div>';
    echo '<label>'.__('Video Embed Code', GKTPLNAME).'</label>';
    echo '<textarea name="kitecom_featured_video" class="widefat">'.$featured_video.'</textarea>';
    echo '<input type="hidden" name="kitecom_featured_video_nonce" id="kitecom_featured_video_nonce" value="'.wp_create_nonce(plugin_basename(__FILE__)).'" />';
    echo '</div>';
}

function kitecom_save_featured_video(){
    global $post;
	// check nonce
    if(!isset($_POST['kitecom_featured_video_nonce']) || !wp_verify_nonce($_POST['kitecom_featured_video_nonce'], plugin_basename(__FILE__))) {
	...
	// if the value exists
    if(isset($_POST['kitecom_featured_video'])) {
	    $featured_video = $_POST['kitecom_featured_video'];
	    
	    if($featured_video != '') {
	    	delete_post_meta($post->ID, '_kitecom-featured-video');
	    	add_post_meta($post->ID, '_kitecom-featured-video', $featured_video);
	    } else {
	    	delete_post_meta($post->ID, '_kitecom-featured-video');
	    }
    }
...
add_action( 'save_post',  'kitecom_save_featured_video' );
add_action( 'admin_menu', 'kitecom_add_featured_video' );
...
add_filter( 'widget_update_callback', 'kitecom_widget_update', 10, 4);

// definition of the additional operation
function kitecom_widget_update($instance, $new_instance, $old_instance, $widget) {	
	global $tpl;
...
function kitecom_widget_control() {	
	...
	if(get_option($tpl->name . '_widget_rules_state') == 'Y') {
		...
		kitecom_widget_control_styles_list($params[0]['widget_id'], $id, $style, $responsiveMode, $usersMode, $style_css);
	} else {
		// get the widget parameters
		$params = func_get_args();
		// find the widget ID
...
add_action( 'sidebar_admin_setup', 'kitecom_add_widget_control'); 

function kitecom_add_widget_control() {	
	global $tpl;
	...
	foreach ( $wp_registered_widgets as $id => $widget ) {	
		...
		$wp_registered_widget_controls[$id]['callback'] = 'kitecom_widget_control';		
	}
}

function kitecom_widget_control_styles_list($widget_name, $id, $value1, $value2, $value3, $value4 = '') {
...
// Add the Meta Box
function kitecom_add_og_meta_box() {
    add_meta_box(
		'kitecom_og_meta_box',
		'Open Graph metatags',
		'kitecom_show_og_meta_box',
		'post',
		'normal',
		'high'
	);
...
if(get_option($tpl->name . '_opengraph_use_opengraph') == 'Y') {
    add_action('add_meta_boxes', 'kitecom_add_og_meta_box');
}

// The Callback
function kitecom_show_og_meta_box() {
	global $tpl, $post;
		...
		echo '<tr>
				...
				switch($field->type) {
					...
					case 'image':
						...
						echo '<span class="kitecom_opengraph_default_image" style="display:none">'.$image.'</span>';
						if ($meta) { 
							$image = wp_get_attachment_image_src($meta, 'medium');	
							$image = $image[0];
						}
						echo	'<input name="'.$field->id.'" type="hidden" class="kitecom_opengraph_upload_image" value="'.$meta.'" />
									<img src="'.$image.'" class="kitecom_opengraph_preview_image" alt="" /><br />
										<input class="kitecom_opengraph_upload_image_button button" type="button" value="Choose Image" />
										<small><a href="#" class="kitecom_opengraph_clear_image">Remove Image</a></small>
...
// Save the Data
function kitecom_save_custom_meta($post_id) {
    global $tpl;
...
add_action('save_post', 'kitecom_save_custom_meta');  

// Add the Meta Box for Twitter cards
function kitecom_add_twitter_meta_box() {
    add_meta_box(
		'kitecom_twitter_meta_box',
		'Twitter Cards metatags',
		'kitecom_show_twitter_meta_box',
		'post',
		'normal',
		'high'
	);
...
if(get_option($tpl->name . '_twitter_cards') == 'Y') {
	add_action('add_meta_boxes', 'kitecom_add_twitter_meta_box');
}

// The Callback for Twiter metabox
function kitecom_show_twitter_meta_box() {
	global $tpl, $post;
	// load custom meta fields
...
function kitecom_save_custom__twitter_meta($post_id) {
    global $tpl;
...
add_action('save_post', 'kitecom_save_custom__twitter_meta');
...
function kitecom_opensearch_head() {
...
function kitecom_opensearch() {
	...
	if (!empty($wp_query->query_vars['opensearch_description']) ) {
		...
		echo "\t<Image width=\"16\" height=\"16\" type=\"image/x-icon\">".kitecom_file_uri('favicon.ico')."</Image>\n";
...
if(get_option($tpl->name . "_opensearch_use_opensearch", "Y") == "Y") {
	add_action('wp_head', 'kitecom_opensearch_head');
	add_action('template_redirect', 'kitecom_opensearch');
	add_filter('query_vars', 'kitecom_opensearch_query_vars');
}
