...
include_once(kitecom_file('kitecom/classes/class.gkobject.php'));
// Including file with template browser class
include_once(kitecom_file('kitecom/classes/class.gkbrowser.php'));
// Including file with menu walker class
include_once(kitecom_file('kitecom/classes/class.menu.walker.php'));
// Including file with mobile menu walker class
include_once(kitecom_file('kitecom/classes/class.menu.walker.mobile.php'));
// Including file used for WPML support
if (defined('ICL_SITEPRESS_VERSION')) {
	include_once(kitecom_file('kitecom/wpml.php'));
}
...
class KitecomWP {
	...
	public function __construct() {
		...
		if(
			...
			if(get_locale() != '' && !is_dir(get_template_directory() . '/kitecom/config/'. get_locale())) {
				array_push($this->problems, 'The kitecom/config/'.get_locale().' directory doesn\'t exist');
			}
			// 
			if(get_locale() != '' && !is_dir(get_template_directory() . '/kitecom/options/'. get_locale())) {
				array_push($this->problems, 'The kitecom/options/'.get_locale().' directory doesn\'t exist');
			}
		}
	 ...
	 * the functions located in kitecom/functions.php file
	 *
	 * @return null
	 *
	 **/
	 ...
	 private function add_actions() {
	 	add_action( 'admin_menu', 'kitecom_admin_menu' );
	 	add_action( 'widgets_init', 'kitecom_widgets_init' );
	 }

	/**
	 *
	 * Function to add template filters connected with
	 * the functions located in kitecom/filters.php file
	 *
	 * @return null
	 *
	 **/
	 ...
	 private function add_filters() {
	 	...
	 	add_filter('wp_die_handler', 'kitecom_die_handler');
	 }
	 ...
	 private function add_features() {
	 	...
	 	$this->register_fonts();
	 	// add metaboxes
	 	add_action('add_meta_boxes', 'add_kitecom_metaboxes' );
	 	add_action('save_post', 'kitecom_metaboxes_save' ); 
	 	// add Gavern Shortcode button to the TinyMCE editor
	 	//add_action('admin_head', 'add_kitecom_shortcode_button');
	...
	**/ 
	...
	public function get_json($dir, $filename, $lang = true) {		
		...
		if (defined('ICL_SITEPRESS_VERSION')) {
			$lang = apply_filters('kitecom-get-json',$dir,$lang);
		}
		$path = kitecom_file('kitecom/' . $dir . '/' . $lang . $filename . '.json');
   ...
   /**
   	...
   	**/ 
   	...
   	function loadWidgetSettings() {
   		...
   		$data_file = get_template_directory() . '/kitecom/demo/widgets.data';
